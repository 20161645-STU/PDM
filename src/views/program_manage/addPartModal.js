...
import { selectPartUrl, makeRelationUrl } from '../../../src/dataModule/UrlList'
...
class AddPartModal extends Component{
  constructor(props){
    super(props);
    this.state = {
      part_no: '',
  ...
  getPartInfo = () => {
    let me = this
    if (me.props.partsInfo !== null && me.findPart(me.props.partsInfo) !== -1) {
  ...
  findPart = (value) => {
    if (value !== null) {
      for (let i = 0; i <= value.length - 1; i++){
        if (value[i]['partNo'] === this.state.part_no * 1) {
          return i
        }
      }
      return -1
  ...
  handleSelectParts = async (params) => {
    ...
    for (let i = 0; i < params.length; i++) {
      returnContent.push( await this.sentPartsMes(params[i].target, params[i].origin))
    }
  ...
  sentPartsMes = (value1, value2) => {
    return new Promise((resolve, reject) => {
      model.fetch(
        { origin: value2, target: value1, origin_type: 'project', target_type: 'tss' },
        makeRelationUrl,
        'post',
        function (res) {
          resolve(res)
...
export default connect(mapStateToProps, null)(AddPartModal)
